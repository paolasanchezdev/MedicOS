...
// DESCRIPCIÓN: Formulario de Login con prevención de multi-clic y
//              soporte para Modo Estación MedicOS (Offline).
// =========================================================================
...
export const LoginForm: React.FC<LoginFormProps> = ({
  ...
  // MODO ESTACIÓN
  // =========================================================================
  // Cuando VITE_STATION_MODE=true:
  // - No se carga Cloudflare Turnstile.
  // - No se exige turnstileToken.
  // - El login se realiza directamente contra la API local de la Raspberry.
  //
  // En producción normal, si la variable no existe o es false,
  // Turnstile continúa funcionando como antes.
  // =========================================================================
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Prevención de multi-clic mientras se procesa el login.
    if (loading) return;

    // En modo normal Turnstile es obligatorio.
    //
    // En modo estación NO se realiza esta validación porque
    // la Raspberry trabaja sin Internet y el backend local
    // ya tiene autorizado el bypass de Turnstile.
    if (!isStationMode && !turnstileToken) {
      alert(
        'Por favor, espera a que se complete la verificación de seguridad anti-bot.'
      );
      return;
    }

    const currentToken = isStationMode ? '' : turnstileToken;
    ...
    <div className="w-full max-w-md mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 text-left">
        ...
        {/* =================================================================
            ...
            IMPORTANTE:
            En Modo Estación no se monta el componente Turnstile.
            Esto evita cualquier intento del navegador de contactar
            Cloudflare cuando la Raspberry está aislada de Internet.
            ================================================================= */}
          ...
          <div className="rounded-xl border border-teal-200 bg-teal-50 px-4 py-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold uppercase tracking-wide text-teal-800">
                Estación MedicOS · Modo Offline
              </span>
            </div>

            <p className="mt-1 text-xs font-medium text-teal-700">
              Conexión local activa. La autenticación se realiza contra
              el servidor de la estación.
            </p>
          </div>
        )}
