// apps/web/src/portals/admin/pages/establecimientos/clinicas/components/ClinicMap.tsx
...
interface ClinicMapProps {
  ...
  onSelectClinic?: (clinic: Establishment) => void;
}
...
const getGoogleMapsUrl = (c: Establishment) => {
  const query = `${c.name}, ${c.municipality || ''}, ${c.department || ''}, El Salvador`;
...
export const ClinicMap: React.FC<ClinicMapProps> = ({
  ...
  const [activeClinicId, setActiveClinicId] = useState<string | undefined>(
    selectedClinicId
  );

  // Estados de Pan & Zoom para el mapa offline
  ...
  const activeClinic =
    clinics.find((c) => c.id === (activeClinicId || selectedClinicId)) || null;

  const handleSelectClinicOffline = (
  ...
  const initLeafletMap = useCallback(() => {
    ...
    clinics.forEach((c) => {
      if (c.latitude != null && c.longitude != null) {
        ...
        marker.addTo(map);
        const gmapsUrl = getGoogleMapsUrl(c);
        marker.bindPopup(`
          <div style="font-family: sans-serif; font-size: 12px; min-width: 200px; line-height: 1.4;">
            <strong style="color: #1e293b; font-size: 13px;">${c.name}</strong><br/>
            <span style="color: #64748b;">📍 ${c.address || `${c.municipality}, ${c.department}`}</span><br/>
            <div style="margin-top: 6px; display: flex; justify-content: space-between; align-items: center;">
              <span style="color: #0f766e; font-weight: 600;">ISSS • ${c.level}</span>
          ...
          >
            ...
            >
              ...
              >
                ...
                {clinics
                  .filter((c) => projectGpsToSvg(c.latitude, c.longitude))
                  .map((c) => {
                    const coords = projectGpsToSvg(c.latitude, c.longitude)!;
                    ...
                    return (
                      <g key={c.id} className="cursor-pointer">
                        ...
                        <circle
                          ...
                          cy={coords.y}
                          r={isSelected ? '6' : '4.5'}
                          fill={c.status === 'OPERATIONAL' ? '#0f766e' : '#d97706'}
                          stroke="#ffffff"
                          strokeWidth={isSelected ? '1.5' : '1'}
                          className="transition-all duration-200 hover:opacity-80"
                          onClick={(e) => {
                            e.stopPropagation();
                            handleSelectClinicOffline(c, coords);
                          }}
                        />
                      </g>
                    );
              ...
              >
                ...
                <div className="text-slate-600 space-y-0.5">
                  <p className="flex items-start gap-1">
                    ...
                    <span className="line-clamp-2">{activeClinic.address || 'Sin dirección'}</span>
                  </p>
                  <p className="text-[11px] text-slate-500">
                    {activeClinic.municipality}, {activeClinic.department}
                  </p>
                  {activeClinic.phone && (
                ...
                <div className="pt-2 border-t border-slate-100 flex items-center justify-between">
                  <span className="text-emerald-700 font-semibold">
                    {activeClinic.status === 'OPERATIONAL' ? '● Operativa' : activeClinic.status}
                  </span>
                  <a
                    href={getGoogleMapsUrl(activeClinic)}
                    target="_blank"
                    rel="noreferrer"
