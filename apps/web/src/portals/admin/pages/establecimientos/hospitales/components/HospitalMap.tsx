// apps/web/src/portals/admin/pages/establecimientos/hospitales/components/HospitalMap.tsx
...
interface HospitalMapProps {
  ...
  onSelectHospital?: (hospital: Establishment) => void;
}
...
const getGoogleMapsUrl = (h: Establishment) => {
  const query = `${h.name}, ${h.municipality || ''}, ${h.department || ''}, El Salvador`;
...
export const HospitalMap: React.FC<HospitalMapProps> = ({
  ...
  // Estados de navegación Pan & Zoom para el modo offline
  ...
  const activeHospital =
    hospitals.find((h) => h.id === (activeHospitalId || selectedHospitalId)) || null;

  // Centrar y hacer zoom en un hospital seleccionado
  const handleSelectHospitalOffline = (
  ...
  // Inicialización del mapa en línea (Leaflet)
  const initLeafletMap = useCallback(() => {
    ...
    hospitals.forEach((h) => {
      if (h.latitude != null && h.longitude != null) {
        ...
        marker.addTo(map);
        const gmapsUrl = getGoogleMapsUrl(h);
        marker.bindPopup(`
          <div style="font-family: sans-serif; font-size: 12px; min-width: 200px; line-height: 1.4;">
            <strong style="color: #1e293b; font-size: 13px;">${h.name}</strong><br/>
            <span style="color: #64748b;">📍 ${h.address || `${h.municipality}, ${h.department}`}</span><br/>
            <div style="margin-top: 6px; display: flex; justify-content: space-between; align-items: center;">
              <span style="color: #059669; font-weight: 600;">Camas: ${h.availableBeds ?? 0}/${h.totalBeds ?? 0}</span>
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-lg overflow-hidden relative z-0 isolate">
      {/* Encabezado */}
      <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
        <div>
          ...
          <p className="text-[11px] text-slate-500">
            {mapMode === 'offline'
              ? 'Arrastre el mapa para moverse o use la rueda del ratón para acercar/alejar'
              : 'Red hospitalaria sincronizada en vivo con OpenStreetMap'}
          </p>
        </div>
      ...
      <div className="w-full h-112.5 relative z-0 isolate bg-slate-50">
        {mapMode === 'online' ? (
          ...
          >
            {/* Controles de Navegación Offline */}
            ...
            {/* Lienzo SVG con Transformación Pan & Zoom */}
            <svg
            ...
            >
              ...
              >
                ...
                {hospitals
                  .filter((h) => projectGpsToSvg(h.latitude, h.longitude))
                  .map((h) => {
                    const coords = projectGpsToSvg(h.latitude, h.longitude)!;
                    const isSelected = h.id === activeHospitalId;

                    return (
                      <g key={h.id} className="cursor-pointer">
                        {/* Halo concéntrico estable para el hospital activo (sin desfasarse) */}
                        {isSelected && (
                          <>
                            <circle
                              cx={coords.x}
                        ...
                        <circle
                          ...
                          cy={coords.y}
                          r={isSelected ? '6' : '4.5'}
                          fill={h.status === 'OPERATIONAL' ? '#059669' : '#d97706'}
                          stroke="#ffffff"
                          strokeWidth={isSelected ? '1.5' : '1'}
                          className="transition-all duration-200 hover:opacity-80"
                          onClick={(e) => {
                            e.stopPropagation();
                            handleSelectHospitalOffline(h, coords);
                          }}
                        />
                      </g>
                    );
              ...
              >
                <div className="flex items-start justify-between gap-1">
                  <div>
                    <span className="text-[10px] font-mono text-teal-700 font-semibold uppercase bg-teal-50 px-1.5 py-0.5 rounded">
                      {activeHospital.code} • Nivel {activeHospital.level}
                    </span>
                ...
                <div className="text-slate-600 space-y-0.5">
                  <p className="flex items-start gap-1">
                    ...
                    <span className="line-clamp-2">{activeHospital.address || 'Sin dirección'}</span>
                  </p>
                  <p className="text-[11px] text-slate-500">
                    {activeHospital.municipality}, {activeHospital.department}
                  </p>
                </div>

                <div className="pt-2 border-t border-slate-100 flex items-center justify-between">
                  <span className="text-slate-500">
                    Camas: <strong className="text-emerald-600">{activeHospital.availableBeds ?? 0}/{activeHospital.totalBeds ?? 0}</strong>
                  </span>
                  <a
                    href={getGoogleMapsUrl(activeHospital)}
                    target="_blank"
                    rel="noreferrer"
