  ...
  return (
    <div className="bg-medicos-surface rounded-2xl border border-medicos-soft-border p-5 shadow-xs flex flex-col justify-between h-full space-y-4">
      ...
      <div className="space-y-2">
        {tienePendientes ? (
          acciones.map((item) => (
            <div
              key={item.id}
              className="p-3.5 rounded-xl bg-medicos-light-bg border border-medicos-soft-border hover:border-medicos-teal/50 transition-all duration-150 flex items-start justify-between gap-3"
            >
              <div className="space-y-0.5">
                <p className="text-xs font-bold text-medicos-dark-blue">{item.titulo}</p>
                <p className="text-[11px] text-medicos-muted leading-snug font-medium">{item.descripcion}</p>
              </div>
              {item.ruta && (
                <Link
                  to={item.ruta}
                  className="p-1.5 rounded-lg bg-medicos-surface border border-medicos-soft-border text-medicos-teal hover:bg-medicos-teal hover:text-white transition-all duration-150 shrink-0"
      ...
      <div className="pt-2 border-t border-medicos-soft-border flex items-center justify-between">
        <span className="text-[11px] text-medicos-muted flex items-center gap-1 font-medium">
          <ClipboardList className="w-3.5 h-3.5 text-medicos-teal" /> Notificaciones operativas
        </span>
        <Link
          to="/paciente/notificaciones"
          className="text-xs font-semibold text-medicos-teal hover:underline transition-all duration-150"
        >
